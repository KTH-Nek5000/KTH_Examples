#########################################################
# Extract statistical fields over a 2D interpolation mesh
# NOTE: active coordinates: x and y 
#########################################################
# Adopted from https://github.com/KTH-Nek5000/KTH_Examples/blob/master/pipe_PSTAT2D/pp_python/MODULES/cartTerms.py
...
def comp(db,nu,rho):
    """
    Compute mean, rms, and the budget terms of the Reynolds stress components
    using Nek5000 statistical data available in the database `db`.

    Args:
       'db': dictionary containing the F (field) and D (derivative) data
       'nu': viscosity (set based on the Nek5000 case)
       'rho': density (set based on the Nek5000 case)

    Returns:
       'dbBudget': dictionary containing the computed terms and coordinates of the
...
#   Save averaged-interpolated fields in a dictionary
    dbBudget = {'U': U, 'V': V,'W': W, \
            'uu':uu, 'vv':vv, 'ww':ww, 'uv':uv, 'uw':uw, 'vw':vw,'TKE':(uu+vv+ww)/2, \
    ...
    #copy (x,y) coordinates of the interpolating points from db to dbBudget
    dbBudget.update({'x':db['x']})
    dbBudget.update({'y':db['y']})
